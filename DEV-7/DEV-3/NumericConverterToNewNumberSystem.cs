...
namespace DEV_3
{
    ...
    public class NumericConverterToNewNumberSystem
    {
        ...
        {
            // if number or basis contains letters or another forbidden symbols
            string pattern = @"[0-9]+";
            if (!Regex.IsMatch(stringOfNumber, pattern, RegexOptions.IgnoreCase))
            {
                throw new Exception("Number contains forbidden symbols!");
            }
            if (!Regex.IsMatch(stringOfBasis, pattern, RegexOptions.IgnoreCase))
            {
                throw new Exception("Basis contains forbidden symbols!");
            }

            // Maximum size of digits is 9
            int maxSizeOfParameter = 9;
            if (stringOfNumber.Length > maxSizeOfParameter)
            {
                throw new Exception("Number is too big! Maximum size of digits is 9");
            }
